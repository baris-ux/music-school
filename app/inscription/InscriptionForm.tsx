...
export default function InscriptionForm({ cours }: { cours: Course[] }) {
  ...
  const [state, action, isPending] = useActionState(soumettreInscription, {});
  ...
  return (
    ...
    >
      ...
      <button
        type="submit"
        disabled={isPending}
        className="inline-flex w-full items-center justify-center rounded-xl bg-slate-900 px-4 py-2.5 text-sm font-medium text-white transition hover:bg-slate-800 disabled:opacity-50 disabled:cursor-not-allowed"
      >
        {isPending ? "Envoi en cours…" : t.submit}
      </button>
    </form>
  );
}